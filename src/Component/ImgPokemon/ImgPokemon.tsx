...
const ImgPokemon:FC<Props> = ({ data, onClickHandler, onCloseHandler, bounding, size }) => {
    ...
    return (
        <>
            <motion.div className="img-pokemon" initial={{opacity:0}} animate={{opacity: 1, transition:{delay: 1, ...transition} }}  >
            ...
            {
                bounding 
                ?  <motion.div className="transition-page" 
                        initial={{top:bounding.top, right: window.innerWidth - bounding.right, width:bounding.width, height:bounding.height, rotateY:"180deg", rotateX:20, opacity: .8}}
                        animate={{top:0, right: 0, width: window.innerWidth > 1440 ? "calc(45% - 1em)": "calc(50% - 1em)", height:"100%", rotateY:0, rotateX:0, opacity: 1, transition:transition}} />
