...
const MainPage:FC<Props> = ({ action, data }) => {
    ...
    const { loading, onClickHandler, urlNext, error } = useListPokemon( url, action)

    const observer = useRef<any>(null)
    const observerElement = useCallback(node => {
        if (loading) return
    ...
    return (
        <main>
            ...
            <section className="main-page-container">
                ...
                <ListPokemon pokemons={pokemons} onClickHandler={onClickHandler} observerElement={observerElement} loading={loading} />
            </section>
        </main>
    )
}
