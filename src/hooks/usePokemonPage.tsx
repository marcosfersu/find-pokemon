...
const usePokemonPage = (
  ...
  allEvolutionPokemon: allPokemonsProps,
  action: any
) => {
      ...
      try {
        ...
        if (data.evolution) {
          if (evolutionPosition === "") {
            if (
              (!data.evolution.prev || data.evolution.prev[0] > 151) &&
              data.evolution.next
            )
              evolutionPosition = "first";
            else if (
              data.evolution.prev &&
              data.evolution.prev[0] < 151 &&
              !data.evolution.next
            )
              evolutionPosition = "last";
            else if (
              data.evolution.prev &&
              data.evolution.prev[0] < 151 &&
              data.evolution.next
            )
              evolutionPosition = "mid";
          }
          ...
          if (evolutionPosition === "first") {
            ...
            setGroupEvolution((groupEvolution: any) => ({
              ...groupEvolution,
              [data.id]: { evolve, evolImage },
            }));
          ...
          if (evolutionPosition === "mid") {
            const evolveNext = data.evolution.next
              ? data.evolution.next[0]
              : null;
            const evolvePrev = data.evolution.prev
              ? data.evolution.prev[0]
              : null;
            const evolImage = data.sprite;
  ...
  useEffect(() => {
    action({
      ...
      payload: { [dataPokemon.id]: { dataPokemon, spritesPokemon } },
    });
  }, [spritesPokemon]);

  useEffect(() => {
    if (groupEvolution.length !== 0)
      action({
        type: `SET_EVOLUTION_POKEMON`,
        payload: { [idPokemon]: groupEvolution },
      });
  }, [groupEvolution]);
